...
greetings() {
  echo -e "${GREEN_COLOR}Lazyadb ${RES}"
  echo -e "${GREEN_COLOR} (h)elp ${RES}查看帮助"
  echo -e "${GREEN_COLOR} (e)xit ${RES}退出软件"
  echo -e "${GREEN_COLOR} (m)enu ${RES}打开菜单"
  # date
}
...
status_line() {
  echo "Lazyadb > connect"
  echo "Lazyadb > android apk manager"
  echo "Lazyadb > file manager"
  echo "Lazyadb > more"
  echo "Lazyadb > config"
}
...
menu_2() {
  select choice in 安装软件 卸载软件 获取包名 提取安装包 打开软件 快速安装 返回
  do
    PS3="请输入你要用的功能:"
    case "$choice" in
      安装软件)
